...
class ExperimentConfig:
    ...
    min_datasets: int = 3             # gene/pathway must appear in >= this many datasets
...
def _standardize_metadata(metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    result['inflammation'] = np.where(result['inflammation'].isna() & (result['disease'].isin(['UC', 'CD'])), 'Inflamed', result['inflammation'])
...
def _select_uc_vs_ctrl(metadata: pd.DataFrame) -> pd.Series:
    ...
    mask = (
        ((metadata['disease'] == GROUP_1) & (metadata['inflammation'] == INFLAMMATION_1))
        | ((metadata['disease'] == GROUP_2) & (metadata['inflammation'] == INFLAMMATION_2))
    ) & time_ok & is_colon
...
def _limma_moderated_t(data: pd.DataFrame, labels: pd.Series) -> pd.DataFrame:
    ...
    try:
        ...
        # Group vector: 0=GROUP_2, 1=GROUP_1
        group = ro.IntVector([1 if labels.loc[s] == GROUP_1 else 0 for s in data.index])
...
def _compute_gene_stats(data: pd.DataFrame, labels: pd.Series) -> pd.DataFrame:
    ...
    """
    disease_mask = labels == GROUP_1
    ctrl_mask = labels == GROUP_2
    ...
    for gene in tqdm(data.columns, desc='Genes', unit='gene', leave=False):
        ...
        disease_vals = data.loc[disease_mask, gene].dropna().astype(float)
        n_ctrl, n_disease = len(ctrl_vals), len(disease_vals)
        if n_ctrl < 2 or n_disease < 2:
            continue

        mean_ctrl, mean_disease = ctrl_vals.mean(), disease_vals.mean()
        g = _hedges_g(mean_disease, mean_ctrl, disease_vals.var(ddof=1), ctrl_vals.var(ddof=1), n_disease, n_ctrl)
        g_var = _hedges_g_variance(g, n_disease, n_ctrl) if not np.isnan(g) else np.nan

        raw_results[gene] = {
            'gene': gene,
            f'n_{GROUP_1.lower()}': n_disease,
            f'n_{GROUP_2.lower()}': n_ctrl,
            f'mean_{GROUP_1.lower()}': mean_disease,
            f'mean_{GROUP_2.lower()}': mean_ctrl,
            f'mean_diff': mean_disease - mean_ctrl,
            'hedges_g': g,
            'hedges_g_var': g_var,
        }
...
def _meta_analyse_genes(per_dataset: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    records = []
...
def _filter_genes(meta: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    return meta[
...
def _compute_rra(per_dataset_genes: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    """
...
def _meta_analyse_pathways(per_dataset: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    records = []
...
def _filter_pathways(meta: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    return meta[
        (meta['datasets'] >= config.min_datasets)
        & (meta['direction_ratio'] >= config.direction_threshold)
        & (meta['q_value'] < config.alpha)
...
def experiment(config: ExperimentConfig) -> None:
    # ---- Load & prepare ----
    ...
    if labels.empty:
        logging.warning(f'No samples matched the {GROUP_1} vs {GROUP_2} selection criteria')
        return
    ...
    for idx, ds_name in enumerate(dataset_names, 1):
        ...
        n_total = len(ds_data)
        n_group_1 = int((ds_labels == GROUP_1).sum())
        n_group_2 = int((ds_labels == GROUP_2).sum())

        if n_total < config.min_samples or n_group_1 < config.min_per_group or n_group_2 < config.min_per_group:
            logging.info('Skipping %s (%d/%d): n=%d %s=%d %s=%d', ds_name, idx, len(dataset_names), n_total, GROUP_1, n_group_1, GROUP_2, n_group_2)
            continue

        logging.info('Processing %s (%d/%d): n=%d %s=%d %s=%d', ds_name, idx, len(dataset_names), n_total, GROUP_1, n_group_1, GROUP_2, n_group_2)
        ...
        dataset_summaries.append({
            'dataset': ds_name,
            'n_total': n_total,
            f'n_{GROUP_1.lower()}': n_group_1,
            f'n_{GROUP_2.lower()}': n_group_2,
